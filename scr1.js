...
function getTypeThumbnail(type) {
  const thumbnailMap = {
    single: "/views/single unit.jpeg",
    double: "/views/double.jpeg",
    triple: "/views/triple.jpeg",
  };
...
function getShapeImage(shape) {
  const imageMap = {
    ...
    triangle:
      "https://images.unsplash.com/photo-1586023492125-27b2c045efd7?w=120&h=120&fit=crop&crop=center",
    ...
    circle:
      "https://images.unsplash.com/photo-1541888946425-d81bb19240f5?w=120&h=120&fit=crop&crop=center",
    other:
      "https://images.unsplash.com/photo-1557804506-669a67965ba0?w=120&h=120&fit=crop&crop=center",
  };
  ...
  ["single", "double", "triple"].forEach((type) => {
    ...
    if (extrasSelect) {
      extrasSelect.addEventListener("change", function () {
        ...
        const georgianOptions = document.getElementById(
          type + "GeorgianOptions"
        );
...
function initializeEventListeners() {
  ...
  document.addEventListener("click", function (e) {
    if (
      e.target.id === "singleEditShape" ||
      e.target.id === "doubleEditShape" ||
      e.target.id === "tripleEditShape"
    ) {
      let tabType;
      ...
      const storedShape =
        selectedShapeData.shape ||
        document.getElementById(tabType + "Shape").value;
      ...
      const mockShapeElement = {
        ...
        querySelector: (selector) => {
          if (selector === "p.text-sm.font-medium") {
            return { textContent: getShapeDisplayName(storedShape) };
          }
          if (selector === "img") {
            return { src: getShapeImage(storedShape) };
          }
          return null;
        },
      };
...
function handleFileUpload(event, type) {
  ...
  files.forEach((file) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      alert(
        `File "${file.name}" is not a supported format. Please use PNG, JPG, PDF, or DOCX.`
      );
      return;
    }
...
function updateFileDisplay(type) {
  ...
  const files =
    window.uploadedFiles && window.uploadedFiles[type]
      ? window.uploadedFiles[type]
      : [];
...
function updateArea(type) {
  ...
  document.getElementById(type + "Area").textContent =
    areaInSquareMeters.toFixed(3) + " m²";
  calculatePrice();
}
...
function calculateSinglePrice() {
  ...
  const shapeModifier =
    parseFloat(
      document.querySelector(".single-shape-option.shape-selected")?.dataset
        .price
    ) || 0;
  price *= 1 + shapeModifier / 100;

  // Polished glass (always yes for single panel, +30%)
  const polishedRadio = document.querySelector(
    'input[name="singlePolished"]:checked'
  );
  if (polishedRadio) {
    const polishedModifier = parseFloat(polishedRadio.dataset.price) || 0;
    price *= 1 + polishedModifier / 100;
  ...
  // Toughened glass (always yes for single panel, +30%)
  const toughenedRadio = document.querySelector(
    'input[name="singleToughened"]:checked'
  );
  if (toughenedRadio) {
    const toughenedModifier = parseFloat(toughenedRadio.dataset.price) || 0;
    price *= 1 + toughenedModifier / 100;
  ...
  const extrasPrice =
    parseFloat(
      extrasSelect.options[extrasSelect.selectedIndex].dataset.price
    ) || 0;
  price += extrasPrice;
...
function calculateDoublePrice() {
  ...
  const outerGlassSelect = document.getElementById("doubleOuterGlass");
  const innerGlassSelect = document.getElementById("doubleInnerGlass");
  const outerThicknessMatch =
    outerGlassSelect.options[outerGlassSelect.selectedIndex].text.match(
      /(\d+(\.\d+)?)mm/
    );
  const innerThicknessMatch =
    innerGlassSelect.options[innerGlassSelect.selectedIndex].text.match(
      /(\d+(\.\d+)?)mm/
    );

  const outerThickness_mm = outerThicknessMatch
    ? parseFloat(outerThicknessMatch[1])
    : 4;
  const innerThickness_mm = innerThicknessMatch
    ? parseFloat(innerThicknessMatch[1])
    : 4;
  ...
  const shapeModifier =
    parseFloat(
      document.querySelector(".double-shape-option.shape-selected")?.dataset
        .price
    ) || 0;
  price *= 1 + shapeModifier / 100;
  ...
  // Toughened glass (percentage)
  const toughenedRadio = document.querySelector(
    'input[name="doubleToughened"]:checked'
  );
  if (toughenedRadio) {
    const toughenedModifier = parseFloat(toughenedRadio.dataset.price) || 0;
    price *= 1 + toughenedModifier / 100;
...
function calculateDoubleWeight() {
  ...
  const outerGlassSelect = document.getElementById("doubleOuterGlass");
  const innerGlassSelect = document.getElementById("doubleInnerGlass");
  const outerThicknessStr =
    outerGlassSelect.options[outerGlassSelect.selectedIndex].text.match(
      /(\d+(\.\d+)?)mm/
    );
  const innerThicknessStr =
    innerGlassSelect.options[innerGlassSelect.selectedIndex].text.match(
      /(\d+(\.\d+)?)mm/
    );

  const outerThickness_mm = outerThicknessStr
    ? parseFloat(outerThicknessStr[1])
    : 4;
  const innerThickness_mm = innerThicknessStr
    ? parseFloat(innerThicknessStr[1])
    : 4;
  ...
  const weight_kg =
    area * ((outerThickness_mm + innerThickness_mm) / 1000) * density;
  return weight_kg;
}
...
function calculateTriplePrice() {
  ...
  const centreThicknessMatch =
    centreGlassSelect.options[centreGlassSelect.selectedIndex].text.match(
      /(\d+(\.\d+)?)mm/
    );
  const innerThicknessMatch =
    innerGlassSelect.options[innerGlassSelect.selectedIndex].text.match(
  ...
  const outerThickness_mm = outerThicknessMatch
    ? parseFloat(outerThicknessMatch[1])
    : 4;
  const centreThickness_mm = centreThicknessMatch
    ? parseFloat(centreThicknessMatch[1])
    : 4;
  const innerThickness_mm = innerThicknessMatch
    ? parseFloat(innerThicknessMatch[1])
    : 4;
  ...
  const totalThickness_m =
    (outerThickness_mm + centreThickness_mm + innerThickness_mm) / 1000;
  const weight_kg = chargeableArea * totalThickness_m * density;
  ...
  const shapeModifier =
    parseFloat(
      document.querySelector(".triple-shape-option.shape-selected")?.dataset
        .price
    ) || 0;
  price *= 1 + shapeModifier / 100;
  ...
  // Toughened glass (percentage)
  const toughenedRadio = document.querySelector(
    'input[name="tripleToughened"]:checked'
  );
  if (toughenedRadio) {
    const toughenedModifier = parseFloat(toughenedRadio.dataset.price) || 0;
    price *= 1 + toughenedModifier / 100;
...
function validateSingleForm() {
  ...
  const shapeValue =
    selectedShapeData.shape || document.getElementById("singleShape").value;
  if (!shapeValue) {
  ...
  const width =
    ...
    parseFloat(document.getElementById("singleWidth").value);
  const height =
    selectedShapeData.height ||
    parseFloat(document.getElementById("singleHeight").value);

  if (!width || width < 100 || width > 2800) {
    alert(
      "Please configure the shape dimensions (width must be between 100-2800mm)"
    );
    isValid = false;
  }
  ...
  if (!height || height < 100 || height > 2800) {
    alert(
      "Please configure the shape dimensions (height must be between 100-2800mm)"
    );
    isValid = false;
  }
  ...
  if (shapeValue === "other") {
    const customShape =
      selectedShapeData.customShape ||
      document.getElementById("singleCustomShape").value.trim();
    if (!customShape) {
...
function validateDoubleForm() {
  ...
  const width =
    selectedShapeData.width ||
    parseFloat(document.getElementById("doubleWidth").value);
  const height =
    selectedShapeData.height ||
    parseFloat(document.getElementById("doubleHeight").value);
  ...
  const shapeValue =
    selectedShapeData.shape || document.getElementById("doubleShape").value;
  if (!shapeValue) {
    alert("Please select and configure a shape");
    isValid = false;
  }
  ...
  if (shapeValue === "other") {
    const customShape =
      selectedShapeData.customShape ||
      document.getElementById("doubleCustomShape").value.trim();
    if (!customShape) {
      alert("Please describe the custom shape");
      isValid = false;
    }
...
function validateTripleForm() {
  ...
  const width =
    selectedShapeData.width ||
    parseFloat(document.getElementById("tripleWidth").value);
  const height =
    selectedShapeData.height ||
    parseFloat(document.getElementById("tripleHeight").value);
  ...
  // Validate shape - check both selectedShapeData and form value
  const shapeValue =
    selectedShapeData.shape || document.getElementById("tripleShape").value;
  if (!shapeValue) {
    alert("Please select and configure a shape");
    isValid = false;
  }
  ...
  if (shapeValue === "other") {
    const customShape =
      selectedShapeData.customShape ||
      document.getElementById("tripleCustomShape").value.trim();
    if (!customShape) {
      alert("Please describe the custom shape");
      isValid = false;
    }
  ...
  if (currentTab === "single") {
    ...
    const customShape =
      shapeValue === "other"
        ...
        : "";
    const width =
      selectedShapeData.width || document.getElementById("singleWidth").value;
    const height =
      selectedShapeData.height || document.getElementById("singleHeight").value;
    const reference =
      selectedShapeData.reference ||
    ...
    details = {
      ...
      height: height || "0",
      c:
        selectedShapeData.c !== undefined
          ? selectedShapeData.c
          : document.getElementById(tabType + "C")?.value || "",
      subShape: selectedShapeData.subShape || "", // for single, double, triple
      reference: reference || "",
      glassType:
        glassTypeSelect.selectedIndex > 0
          ? glassTypeSelect.options[glassTypeSelect.selectedIndex].text
          : "Not selected",
      glassThickness:
        glassTypeSelect.dataset.selectedThickness || "Not specified",
      ...
      polished: polishedRadio
        ? polishedRadio.value === "yes"
          ? "Yes"
          : "No"
        : "Yes",
      toughened: toughenedRadio
        ? toughenedRadio.value === "yes"
          ? "Yes"
          : "No"
        : "Yes",
      petFlap: document.getElementById("singlePetFlap").checked ? "Yes" : "No",
      extras:
        extrasSelect.selectedIndex > 0
          ? extrasSelect.options[extrasSelect.selectedIndex].text
          : "None",
    ...
    details = {
      ...
      c:
        selectedShapeData.c !== undefined
          ? selectedShapeData.c
          : document.getElementById("doubleC")?.value || "",
      subShape: selectedShapeData.subShape || "",
      reference: document.getElementById("doubleReference").value || "",
      outerGlass:
        outerGlassSelect.selectedIndex > 0
          ? outerGlassSelect.options[outerGlassSelect.selectedIndex].text
          : "Not selected",
      outerGlassPattern: outerGlassSelect.dataset.selectedPattern || null,
      innerGlass:
        innerGlassSelect.selectedIndex > 0
          ? innerGlassSelect.options[innerGlassSelect.selectedIndex].text
          : "Not selected",
      innerGlassPattern: innerGlassSelect.dataset.selectedPattern || null,
      spacerWidth:
        spacerWidthSelect.selectedIndex > 0
          ? spacerWidthSelect.options[spacerWidthSelect.selectedIndex].text
          : "Not selected",
      spacerColor:
        spacerTypeSelect.selectedIndex >= 0
          ? spacerTypeSelect.options[spacerTypeSelect.selectedIndex].text
          : "Silver",
      shape: getShapeDisplayName(shapeValue) || "Not selected",
      customShape: customShape,
      toughened: toughenedRadio
      ...
      extras:
        extrasSelect.selectedIndex > 0
          ...
          : "None",
      uploadedFiles:
        window.uploadedFiles && window.uploadedFiles["double"]
          ? [...window.uploadedFiles["double"]]
          : [],
    };
  } else if (currentTab === "triple") {
    ...
    details = {
      ...
      outerGlassPattern: outerGlassSelect.dataset.selectedPattern || null,
      centreGlass:
        centreGlassSelect.selectedIndex > 0
          ? centreGlassSelect.options[centreGlassSelect.selectedIndex].text
          : "Not selected",
      ...
      innerGlassPattern: innerGlassSelect.dataset.selectedPattern || null,
      spacer1Width:
        spacer1WidthSelect.selectedIndex > 0
          ? spacer1WidthSelect.options[spacer1WidthSelect.selectedIndex].text
          : "Not selected",
      spacer2Width:
        spacer2WidthSelect.selectedIndex > 0
          ? spacer2WidthSelect.options[spacer2WidthSelect.selectedIndex].text
          : "Not selected",
      spacerColor:
        spacerTypeSelect.selectedIndex >= 0
          ? spacerTypeSelect.options[spacerTypeSelect.selectedIndex].text
      ...
      extras:
        extrasSelect.selectedIndex > 0
          ...
          : "None",
      uploadedFiles:
        window.uploadedFiles && window.uploadedFiles["triple"]
          ? [...window.uploadedFiles["triple"]]
          : [],
    };
  }
...
function clearForm() {
  ...
  if (currentTab === "single") {
    ...
    document
      .querySelectorAll('input[name="singlePolished"]')
      .forEach((radio) => {
        radio.checked = radio.value === "yes";
      });
    document
      .querySelectorAll('input[name="singleToughened"]')
      .forEach((radio) => {
        radio.checked = radio.value === "yes";
      });
    document.getElementById("singlePetFlap").checked = false;
    document
      .querySelectorAll(".single-shape-option")
      .forEach((opt) => opt.classList.remove("shape-selected"));
    ...
    document
      .querySelectorAll(".double-shape-option")
      .forEach((opt) => opt.classList.remove("shape-selected"));
    ...
    document
      .querySelectorAll(".triple-shape-option")
      .forEach((opt) => opt.classList.remove("shape-selected"));
...
function updateCartDisplay() {
  ...
  cart.forEach((item, index) => {
    ...
    const area =
      (parseFloat(item.details.width) / 1000) *
      (parseFloat(item.details.height) / 1000);
    ...
    cartHTML += `
      ...
      item.type
    } glass" class="w-16 h-16 object-cover rounded-lg border-2 border-gray-200">
                            </div>
                            <div class="flex-1">
      ...
      item.type === "single" ? "Unit" : "Glazed"
    }</h4>
                                        <p class="text-sm text-gray-600">${
                                          item.details.width
                                        }mm × ${
      item.details.height
    }mm (${area.toFixed(3)} m²)</p>
                                        ${
                                          item.details.reference
                                            ? `<p class="text-sm text-gray-500">Ref: ${item.details.reference}</p>`
                                            : ""
                                        }
                                    </div>
      ...
      <div>
        <button
          ...
          ${index === 0 ? "disabled" : ""}
          class="inline-flex items-center justify-center w-8 h-8 rounded-full border border-gray-300 bg-white text-blue-600 hover:bg-blue-50 disabled:opacity-50 disabled:cursor-not-allowed mr-1"
          title="Move Up"
        >
        ...
        <button
          onclick="moveCartItem(${index}, 1)"
          ${index === cart.length - 1 ? "disabled" : ""}
          class="inline-flex items-center justify-center w-8 h-8 rounded-full border border-gray-300 bg-white text-blue-600 hover:bg-blue-50 disabled:opacity-50 disabled:cursor-not-allowed"
          title="Move Down"
        >
          <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
...
function getItemDisplayDetails(item) {
  ...
  if (item.type === "single") {
    ...
    if (item.details.c && item.details.c !== "") {
      details.push(`C: ${item.details.c} mm`);
    }
    if (item.details.subShape) {
      details.push(`Sub Shape: ${item.details.subShape}°`);
    }
    // Check if configuration is already in the glass type name
    if (
      !glassInfo.includes("(") &&
      item.details.glassThickness &&
      item.details.glassThickness !== "Not specified"
    ) {
      glassInfo += ` (${item.details.glassThickness}`;
      if (
        item.details.glassCorner &&
        item.details.glassCorner !== "Not specified"
      ) {
    ...
    // Shape
    const shapeDisplay =
      item.details.customShape && item.details.customShape.trim()
        ? item.details.customShape
        : item.details.shape;
    ...
    if (item.type === "double") {
      ...
      const outerThicknessMatch =
        item.details.outerGlass.match(/(\d+(\.\d+)?)mm/);
      const innerThicknessMatch =
        item.details.innerGlass.match(/(\d+(\.\d+)?)mm/);
      ...
      const outerThicknessMatch =
        item.details.outerGlass.match(/(\d+(\.\d+)?)mm/);
      const centreThicknessMatch =
        item.details.centreGlass.match(/(\d+(\.\d+)?)mm/);
      const innerThicknessMatch =
        item.details.innerGlass.match(/(\d+(\.\d+)?)mm/);
    ...
    const outerGlassDisplay = item.details.outerGlass.includes("(")
      ...
      : `${item.details.outerGlass}${
          item.details.outerGlassPattern
            ? ` (${item.details.outerGlassPattern})`
            : ""
        }`;
    ...
    if (
      ...
      item.details.centreGlass !== "Not selected"
    ) {
      const centreGlassDisplay = item.details.centreGlass.includes("(")
        ? item.details.centreGlass
        : `${item.details.centreGlass}${
            item.details.centreGlassPattern
              ? ` (${item.details.centreGlassPattern})`
              : ""
          }`;
    ...
    const innerGlassDisplay = item.details.innerGlass.includes("(")
      ...
      : `${item.details.innerGlass}${
          item.details.innerGlassPattern
            ? ` (${item.details.innerGlassPattern})`
            : ""
        }`;
    ...
    // Spacer information
    if (
      item.details.spacerWidth &&
      item.details.spacerWidth !== "Not selected"
    ) {
      details.push(
        `Spacer: ${item.details.spacerWidth} ${item.details.spacerColor}`
      );
    } else if (
      item.details.spacer1Width &&
      item.details.spacer1Width !== "Not selected"
    ) {
      details.push(
        `Spacers: ${item.details.spacer1Width} & ${item.details.spacer2Width} ${item.details.spacerColor}`
      );
    }
...
function updateCartSummary() {
  ...
  if (deliveryCharge > 0) {
    ...
    shippingLabel.textContent =
      selectedDeliveryArea === 1 ? "Delivery (Area 1):" : "Delivery (Area 2):";
...
function initializeDeliveryModal() {
  ...
  document.querySelectorAll(".delivery-area").forEach((area) => {
    area.addEventListener("click", function () {
      document
        .querySelectorAll(".delivery-area")
...
function confirmPatternSelection() {
  if (pendingPatternSelection && selectedPattern) {
    ...
    const selectedOption =
      pendingPatternSelection.options[pendingPatternSelection.selectedIndex];
...
function initializeShapeDimensionsModal() {
  ...
  if (confirmBtn)
    confirmBtn.addEventListener("click", confirmShapeDimensionsSelection);
...
function showShapeDimensionsModal(shapeElement) {
  ...
  // In showShapeDimensionsModal, pre-fill modalC if exists
  const existingC = document.getElementById(pendingShapeSelection + "C")?.value;
  if (existingC) {
    document.getElementById("modalC").value = existingC;
  } else {
    document.getElementById("modalC").value = "";
  }

  // ...existing code in showShapeDimensionsModal...

  // Sub-shape (rotation) selection
  const subShapeSection = document.getElementById("modalSubShapeSection");
  const subShapeOptions = document.getElementById("modalSubShapeOptions");
  subShapeOptions.innerHTML = ""; // Clear previous

  const shapeKey =
    shapeData.shape in subShapeImages ? shapeData.shape : "default";
  const subShapes = subShapeImages[shapeKey];

  Object.entries(subShapes).forEach(([deg, imgSrc]) => {
    const div = document.createElement("div");
    div.className =
      "sub-shape-option cursor-pointer border-2 border-gray-200 rounded-lg p-2 text-center";
    div.dataset.rotation = deg;
    div.innerHTML = `
    <img src="${imgSrc}" alt="${deg}°" class="w-20 h-20 mx-auto mb-1">
    <span class="block text-xs">${deg}°</span>
  `;
    // Highlight if already selected
    if (selectedShapeData.subShape === deg) {
      div.classList.add("border-blue-500", "ring-2");
    }
    div.onclick = function () {
      document
        .querySelectorAll(".sub-shape-option")
        .forEach((o) => o.classList.remove("border-blue-500", "ring-2"));
      this.classList.add("border-blue-500", "ring-2");
      selectedShapeData.subShape = deg;
    };
    subShapeOptions.appendChild(div);
  });

  // If no previous selection, default to 90°
  if (!selectedShapeData.subShape) {
    selectedShapeData.subShape = "90";
    subShapeOptions
      .querySelector('[data-rotation="90"]')
  ...
  document.getElementById(
    "modalShapePrice"
  ).textContent = `+${shapeData.price}% price modifier`;

  // Show/hide custom shape input
  const customShapeContainer = document.getElementById(
    "modalCustomShapeContainer"
  );
  ...
  const existingWidth = document.getElementById(
    pendingShapeSelection + "Width"
  ).value;
  const existingHeight = document.getElementById(
    pendingShapeSelection + "Height"
  ).value;
  const existingReference = document.getElementById(
    pendingShapeSelection + "Reference"
  ).value;
  const existingCustomShape = document.getElementById(
    pendingShapeSelection + "CustomShape"
  ).value;
  ...
  if (existingWidth && existingHeight) {
    ...
    selectedShapeData.uploadedFiles =
      window.uploadedFiles && window.uploadedFiles[pendingShapeSelection]
        ? [...window.uploadedFiles[pendingShapeSelection]]
        : [];
  ...
  document
    .querySelectorAll("#shapeDimensionsModal .error-message")
    .forEach((error) => error.classList.add("hidden"));
...
function confirmShapeDimensionsSelection() {
  ...
  const cValue = document.getElementById("modalC").value
    ? parseFloat(document.getElementById("modalC").value)
    : "";
...
function updateMainFormFromModal() {
  ...
  document.getElementById(tabType + "C").value =
    selectedShapeData.c !== undefined ? selectedShapeData.c : "";
  document.getElementById(tabType + "Reference").value =
    selectedShapeData.reference;
  ...
  /* if (shapeSummary) {
    ...
    const shapeDimensions = document.getElementById(
      tabType + "SelectedDimensions"
    );
    const shapeArea = document.getElementById(tabType + "SelectedArea");
    const referenceElement = document.getElementById(
      tabType + "SelectedReference"
    );
    const referenceText = document.getElementById(
      tabType + "SelectedReferenceText"
    );

    if (shapeName) {
      const displayName =
        selectedShapeData.shape === "other"
          ? selectedShapeData.customShape
          : getShapeDisplayName(selectedShapeData.shape);
    ...
    if (shapeArea) {
      const area =
        (selectedShapeData.width / 1000) * (selectedShapeData.height / 1000);
  ...
  if (shapeSummary) {
    ...
    if (shapeDimensions) {
      let dims = `${selectedShapeData.width}mm × ${selectedShapeData.height}mm`;
      if (selectedShapeData.c) dims += ` × ${selectedShapeData.c}mm`;
      if (selectedShapeData.subShape)
        dims += `, ${selectedShapeData.subShape}°`;
      shapeDimensions.textContent = dims;
    }
    // Build dimensions string with C if present
    let dims = `${selectedShapeData.width}mm × ${selectedShapeData.height}mm`;
    if (
      selectedShapeData.c !== undefined &&
      selectedShapeData.c !== "" &&
      !isNaN(selectedShapeData.c)
    ) {
      dims += ` × ${selectedShapeData.c}mm`;
    }
    if (shapeDimensions) {
      shapeDimensions.textContent = dims;
    }
    const subShapeSpan = document.getElementById(tabType + "SubShape");
    if (subShapeSpan) {
      if (selectedShapeData.subShape && selectedShapeData.subShape !== "") {
        subShapeSpan.textContent = selectedShapeData.subShape + "°";
      } else {
        subShapeSpan.textContent = "-";
      }
    }
  ...
  if (shapeClass) {
    ...
    const selectedShapeElement = document.querySelector(
      `.${shapeClass}[data-shape="${selectedShapeData.shape}"]`
    );
...
function initializeGlassConfigModal() {
  ...
  document.addEventListener("click", function (e) {
    if (e.target.closest(".thickness-option")) {
      document
        .querySelectorAll(".thickness-option")
        .forEach((opt) => opt.classList.remove("selected"));
      e.target.closest(".thickness-option").classList.add("selected");
      selectedGlassConfig.thickness =
        e.target.closest(".thickness-option").dataset.thickness;
      updateConfirmButton();
    }
  });
  ...
  document.addEventListener("click", function (e) {
    if (e.target.closest(".tinted-color-option")) {
      document
        .querySelectorAll(".tinted-color-option")
        .forEach((opt) => opt.classList.remove("selected"));
      e.target.closest(".tinted-color-option").classList.add("selected");
      selectedGlassConfig.tintedColor = e.target.closest(
        ".tinted-color-option"
      ).dataset.color;
      updateConfirmButton();
    }
  ...
  document.addEventListener("click", function (e) {
    if (e.target.closest(".corner-option")) {
      ...
      selectedGlassConfig.corner =
        e.target.closest(".corner-option").dataset.corner;
      updateConfirmButton();
    }
  });
}
...
function showGlassConfigModal(selectElement) {
  ...
  document
    .querySelectorAll(".thickness-option, .tinted-color-option, .corner-option")
    .forEach((opt) => opt.classList.remove("selected"));
...
function updateConfirmButton() {
  const confirmBtn = document.getElementById("confirmGlassConfig");
  const glassType = pendingGlassConfigSelection
    ? pendingGlassConfigSelection.value
    : "";
...
function hideGlassConfigModal() {
  ...
  if (
    pendingGlassConfigSelection &&
    (!selectedGlassConfig.thickness || !selectedGlassConfig.corner)
  ) {
...
function confirmGlassConfigSelection() {
  if (
    ...
    pendingGlassConfigSelection.dataset.selectedThickness =
      selectedGlassConfig.thickness;
    pendingGlassConfigSelection.dataset.selectedCorner =
      selectedGlassConfig.corner;

    if (selectedGlassConfig.tintedColor) {
      pendingGlassConfigSelection.dataset.selectedTintedColor =
        selectedGlassConfig.tintedColor;
    }

    // Update the display text to show configuration
    const selectedOption =
      pendingGlassConfigSelection.options[
        pendingGlassConfigSelection.selectedIndex
      ];
...
const subShapeImages = {
  square: {
    90: "/views/square-90.png",
    180: "/views/square-180.png",
    270: "/views/square-270.png",
    360: "/views/square-360.png",
  },
  triangle: {
    90: "/views/triangle-90.png",
    180: "/views/triangle-180.png",
    270: "/views/triangle-270.png",
    360: "/views/triangle-360.png",
  },
  rake: {
  ...
  arched: {
    90: "/views/arched-90.png",
    180: "/views/arched-180.png",
    270: "/views/arched-270.png",
    360: "/views/arched-360.png",
  },

  // ...repeat for other shapes...
  // fallback if needed
  default: {
    90: "/views/default-90.png",
    180: "/views/default-180.png",
    270: "/views/default-270.png",
    360: "/views/default-360.png",
  },
};
